...
export function WorkflowAuditInspector({
  ...
  const [statuses, setStatuses] = useState<WorkflowStatusEntry[] | undefined>(
    undefined,
  );
    ...
    const subscription = client
      ...
      .subscribe((update) => {
        if (update.type === "mutation" && update.result) {
          setStatuses(
            (update.result.statuses as WorkflowStatusEntry[]) ?? undefined,
          );
        }
      });
          ...
          <Stack space={1}>
            {sortedStatuses.map((entry, index) => {
              const userLookupValue =
                ...
                entry.completedBy?.email;
              const resolvedActor =
                typeof userLookupValue === "string" && userLookupValue.trim()
                  ? projectUsersByIdentity.get(
                      normalizeAuditLookupValue(userLookupValue),
                    )
                  : undefined;
