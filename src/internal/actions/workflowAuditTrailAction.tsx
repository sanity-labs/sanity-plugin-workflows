...
function createAuditTrailPublishAction(
  ...
  const AuditTrailPublishAction: DocumentActionComponent = (
    props: DocumentActionProps,
  ) => {
    ...
    return {
      ...
      onHandle: async () => {
        ...
        try {
          const document = await client.fetch<null | Record<string, unknown>>(
            `*[_id == $id][0]`,
            {
              id: publishedId,
            },
        ...
        try {
          workflowDefinition = await fetchWorkflowDefinition(
            client,
            documentType,
          );
        } catch {
        ...
        const targetStage = findWorkflowTransitionTarget(
          workflowDefinition,
          draftStatus,
        );

        if (
          !shouldSkipPublishAuditEntry(workflowDocument.statuses, draftStatus)
        ) {
          try {
            ...
            console.error(
              "[workflowAuditTrailAction] Failed to append audit entry:",
              error,
            );
          }
        }
        ...
        if (targetStage?.taskTemplates?.length) {
          try {
            ...
            console.error(
              "[workflowAuditTrailAction] Task creation safety net failed:",
              error,
            );
          }
        }
