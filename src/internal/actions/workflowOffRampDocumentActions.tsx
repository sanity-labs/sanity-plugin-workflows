...
function insertAfterLeadingCustomActions(
  ...
  while (
    leadingCustomActionCount < rest.length &&
    !rest[leadingCustomActionCount]?.action
  ) {
...
async function unpublishDocumentIfNeeded({
  ...
  try {
    ...
    console.error(
      `${logPrefix} Failed to unpublish off-ramped document:`,
      error,
    );
  }
}
...
export function createWorkflowOffRampSlotAction(
  ...
  const WorkflowOffRampSlotAction: DocumentActionComponent = (
    props: DocumentActionProps,
  ) => {
    const { draft, id, onComplete, published, ready, transactionSyncLock } =
      props;
    ...
    const canUseOffRamp = offRamp
      ? canUseOffRampStage({
          ...
          currentUserEmail: (
            currentUser as { email?: string } | null | undefined
          )?.email,
      ...
      : false;
    const isAlreadyInOffRamp = Boolean(
      offRamp?.slug && currentStatus === offRamp.slug,
    );
