...
type DocumentSchemaType = Extract<
  SchemaTypeDefinition,
  { type: "document" }
> & {
...
export function withWorkflow(
  ...
  return (schemaTypes: SchemaTypeDefinition[]): SchemaTypeDefinition[] =>
    schemaTypes.map((schemaType) => {
      ...
      const hasStatuses = existingFields.some(
      ...
      const defaultGroup =
        (existingGroups as Array<{ default?: boolean; name: string }>).find(
          (group) => group.default,
      ...
      const publishGatingRule = (
        ...
        rule.custom(async (_value: unknown, context: ValidationContext) => {
          ...
          const currentStage = allStages.find(
            (stage: { statusSlug: string }) =>
              stage.statusSlug === currentStatus,
          );
          ...
          const firstPublishable = (workflowDefinition.stages || []).find(
            (stage: { enablePublishing: boolean; statusSlug: string }) =>
              stage.enablePublishing,
          );
